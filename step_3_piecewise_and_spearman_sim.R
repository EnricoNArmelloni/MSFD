...
sel="total"## alternatives: "total"; "sizethreshold"
...
if(stand=="Y"){
  ...
  if(is.na(pop_sym)==FALSE){
    
    #### Get ref points
    BRP= read_csv(paste0(dir_t,"pop_sim/",indicator,"RP.csv"))%>%
      dplyr::select("Indicator"= "val", F_ref)%>%
      dplyr::mutate(Indicator=Indicator*10)
  ...
  baseplot=function(dat, clr, lbl){
    ...
    ggplot(data=dat %>%dplyr::filter(source=="observed"))+
      ...
      geom_segment(x=min(BRP$year),xend=max(BRP$year),
                   y=BRP[as.numeric(str_remove(BRP$F_ref, "F"))== min(as.numeric(str_remove(BRP$F_ref, "F"))),][[1]], 
                   yend=BRP[as.numeric(str_remove(BRP$F_ref, "F"))==min(as.numeric(str_remove(BRP$F_ref, "F"))),][[1]],
                   size=1, linetype = "dashed", color="black")+
      annotate("text", x=min(dat$year+spaz), y=BRP[as.numeric(str_remove(BRP$F_ref, "F"))== min(as.numeric(str_remove(BRP$F_ref, "F"))),][[1]]+1.8, label= expression(paste("F" ["0"])))+
      geom_segment(x=min(BRP$year),xend=max(BRP$year),
                   y=BRP[as.numeric(str_remove(BRP$F_ref, "F"))== max(as.numeric(str_remove(BRP$F_ref, "F"))),][[1]], 
                   yend=BRP[as.numeric(str_remove(BRP$F_ref, "F"))==max(as.numeric(str_remove(BRP$F_ref, "F"))),][[1]],
                   size=1, linetype = "dashed", color="black")+
      annotate("text",
               x=min(dat$year+spaz),
               y=BRP[as.numeric(str_remove(BRP$F_ref, "F"))== max(as.numeric(str_remove(BRP$F_ref, "F"))),][[1]]-1.8, 
               label= expression(paste("F" ["01"])))+
  ...
  if(regr.p >= 0.05){
    ...
    pl=baseplot(dataplot, "blue", "BPA result: AP = Ref Point")
    
  }else if(regr.p < 0.05){
    
    if(meanAP > meanBRP){
      
      pl=baseplot(dataplot, "green", "BPA result: AP > Ref Point")
      
    }else{
      
      pl=baseplot(dataplot, "red", "BPA result: AP < Ref Point")
