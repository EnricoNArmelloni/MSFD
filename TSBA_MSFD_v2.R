...
if(sel_ind=="L95"){
  ...
  Ind=Biom
  haul=BIOMhaul%>%dplyr::rename("Year"="year", "Val_haul"="W_sqkm", "stratum"="strata")
  unc=BIOMunc
}
...
ggplot() + geom_line(data=Ind,aes(x=Yr,y=Val)) + geom_hline(data=Ind, aes(yintercept = mean(Ind$Val)), color="red")+ggtitle(paste0(sel_ind, " from observed population,", species," " ,area))+ylab("cm")+xlab("Year")+theme_bw()
ggsave(paste0(mydir_out, "/", sel_ind, species,  ".png"))
...
png(filename=paste0(mydir_out, "/",  sel_ind, species,  "BP_selection.png"))
  plot(bp.msfd)
dev.off()
...
dt<-haul%>%
  dplyr::left_join(., bind_rows(AP%>%dplyr::mutate(RP="AP"), BRP%>%dplyr::mutate(RP="BRP")), by="Year")%>%dplyr::filter(is.na(RP)==FALSE, is.na(Val_haul)==FALSE)

dt$Year <- factor(dt$Year ) 

Mlme1 <- lme(Val_haul ~ RP + stratum, random = ~ 1 | Year, data = dt)

ggplot(data=tibble(Residuals=resid(Mlme1, type="normalized"), Fitted=fitted(Mlme1)))+ 
  geom_point(aes(x=Fitted, y=Residuals))+
  geom_hline(yintercept = 0, color="red")+theme_bw()
ggsave(paste0(mydir_out, "/", sel_ind, species,  "_Period_comparison_Residuals.png"), width=200, units="mm")  

pval=as.data.frame(summary(Mlme1)[["tTable"]])%>%
  dplyr::mutate(nm=rownames(.))%>%
  dplyr::filter(nm=="RPBRP")%>%
...
#library(lme4)
#library(lmerTest)
#res2 = lmer(Val_haul ~ RP + stratum + (1|Year), data = dt, REML=TRUE)
#
#pval=as.data.frame(summary(res2)[["coefficients"]])%>%
#  dplyr::mutate(nm=rownames(.))%>%
#  dplyr::filter(nm=="RPBRP")%>%
#  dplyr::select('Pr(>|t|)')
...
if(pval[[1]] >= 0.05){
  ...
  BPA= 0
}else if(pval[[1]] < 0.05 & mean(AP$Val) > mean(BRP$Val)){
...
#coef=reg[["coefficients"]][2,4] 
#slop=reg[["coefficients"]][2,1]

APTA=dt%>%dplyr::mutate(Year=as.numeric(as.character(Year)))%>%dplyr::filter(Year >= max(Year)-4)

Mlme2=lme(Val_haul ~   Year, random = ~ 1 | stratum, data = APTA)
reg=summary(Mlme2)

ggplot(data=tibble(Residuals=resid(Mlme2, type="normalized"), Fitted=fitted(Mlme2)))+ 
  geom_point(aes(x=Fitted, y=Residuals))+
  geom_hline(yintercept = 0, color="red")+theme_bw()
ggsave(paste0(mydir_out, "/", sel_ind, species,  "_Regression_Residuals.png"), width=200, units="mm")  
...
if(BPA == -2){
  ...
  dfpar=unc%>%dplyr::mutate(VMax=Val+stdev_tot, VMin=Val-stdev_tot)
  ...
  if(BPA == 0){
    ...
    plotfin=plotfin+
      ...
      theme_classic()+
      annotate("text", x=max(dfpar$Year-spaz), y=max(dfpar$VMax)-((max(dfpar$VMax)-min(dfpar$VMin))*0.02), label= "BPA result: AP = RP") 
  ...
  if(TA==0) {
    pf=plotfin+geom_smooth(method = "lm", se = TRUE, color="black", linetype="dashed", size=1.5, data=dfpar%>%dplyr::filter(Year >= max(Year)-4))+
      annotate("text", x=max(dfpar$Year-spaz), y=max(dfpar$VMax)-((max(dfpar$VMax)-min(dfpar$VMin))*0.07), label= "TA result: no sign trend in recent years") 
  }else if(TA == 1){
    pf=plotfin+geom_smooth(method = "lm", se = TRUE, color="green", size=1.5, data=dfpar%>%dplyr::filter(Year>= max(Year)-4))+
      annotate("text", x=max(dfpar$Year-spaz),y=max(dfpar$VMax)-((max(dfpar$VMax)-min(dfpar$VMin))*0.07), label= "TA result: positive trend in recent years") 
  }else if(TA == -1){
    pf=plotfin+geom_smooth(method = "lm", se = TRUE, color="red", size=1.5, data=dfpar%>%dplyr::filter(Year >= max(Year)-4))+
      annotate("text", x=max(dfpar$Year-spaz), y=max(dfpar$VMax)-((max(dfpar$VMax)-min(dfpar$VMin))*0.07), label= "TA result: negative trend in recent years") 
  }
  pf+ggtitle(paste(species, area, sel_ind))+ annotate("text", x=max(dfpar$Year-spaz), y=max(dfpar$VMax)-((max(dfpar$VMax)-min(dfpar$VMin))*0.12), label= res) 
  ggsave(paste0(mydir_out, "/",  sel_ind, species,  "summary.png"), width=200, units="mm")  
}
