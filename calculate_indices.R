...
ggplot()+ geom_line(data=df, aes(x=year, y=Ratio))+ geom_hline(data=qnt, aes(yintercept=percent, color=Percentile))+ ylab("Landings/Survey Index")+ ggtitle(species)

##### D3 - C2 biomass
qnt<-tibble(percent=quantile(Biomass$total_biomass, c(0.33, 0.66)), Percentile=c("0.33", "0.66"))
ggplot()+ geom_line(data=Biomass, aes(x=year, y= total_biomass))+ geom_hline(data=qnt, aes(yintercept=percent, color=Percentile))+ ylab("Biomass (kg/km2)")+ ggtitle(species)
...
ggplot() + geom_line(data=percentiledf,aes(x=Year,y=perc95)) + geom_hline(data=percentiledf_mean, aes(yintercept = mean_95perc), color="blue")+ ggtitle(species)+ ylab("0.95 percentile lenght (mm)")
