...
if(stand=="Y"){
  ...
  # regression
  #my.seg <- segmented(lm0, seg.Z = ~ year , npsi=2)
  
  my.seg1 <- segmented(lm0, seg.Z = ~ year , npsi=1)
  my.seg2 <- segmented(lm0, seg.Z = ~ year , npsi=2)
  
  #if(class(my.seg1)[1]=="segmented"){
  #  if(class(my.seg2)[1]=="segmented"){
  #    
  #    sl=data.frame(AIC(my.seg1, my.seg2))%>%
  #      dplyr::mutate(mod=row.names(.))
  #    my.seg.sel=sl[sl$AIC==min(sl$AIC),][3]
  #    
  #    if(my.seg.sel=="my.seg1"){
  #      my.seg=my.seg1
  #    }else{
  #      my.seg=my.seg2
  #    }
  #    
  #  }else if(class(my.seg2)[1]!="segmented"){
  #    
  #    my.seg=my.seg1
  #    
  #  
  ...
  if((is.na(bp[1])==TRUE)==TRUE){
    ...
    brp<-c(min(data$year), as.vector(bp2), max(data$year))
    Lst<-list()
    
    for(i in 1:(n_breaks+1)){
      RP<-tibble(Indicator=data[data$year %in% seq(brp[i],brp[i+1]),]$Indicator, year=data[data$year %in% seq(brp[i],brp[i+1]),]$year, RP=i)
      Lst[[i]]<-RP
    }
    BRP = rbindlist(Lst)%>%
      arrange(year, RP)%>%
      dplyr::filter(RP!=(max(n_breaks)+1))%>%
      dplyr::distinct(year,.keep_all=TRUE)%>%
      group_by(RP)%>%
      summarize(mean=mean(Indicator))%>%
